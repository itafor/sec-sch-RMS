...
<?php include("includes/adminHead.php"); ?>
 <?php include("includes/adminMenu.php"); ?>
   ...
   <h4 class="page-head-line">Primary School Subjects REGISTRATION FORM</h4>
  <h2> <a href="reg_primary_students.php">Register all students (old)</a></h2> </br></br>   
 <?php
  if(isset($_POST['submit'])){
   ...
   $failure= "Please enter term";
    ...
    $chk_students=mysqli_query($connection,"SELECT * FROM  students WHERE  matricno='$reg_no' AND dept='Primary' ");
  if(mysqli_num_rows($chk_students) <= 0){
    ...
    foreach($_POST['subject'] as $key => $chkcourse){ 
           $sel=mysqli_query($connection,"SELECT * FROM  primary_register WHERE  regno='$reg_no' AND  subject='".$chkcourse."' AND  dept='$dept' AND year='$year' AND term='$term' ");
  ...
  if(mysqli_num_rows($sel) >= 1) {
    ...
    $query=mysqli_query($connection,"INSERT INTO primary_register (id,fullname,regno,year,class,dept,term,subject,exam,ca,regdate) 
    VALUES('','$fullname','$reg_no','$year','$class','$dept','$term','".mysqli_real_escape_string($connection, $sci_subj)."','00','00','$regdate')");
...
<div class="row">  
 ...
 $query =$connection->query("SELECT * FROM primary_subject"); 
