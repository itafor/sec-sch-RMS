...
<?php include("includes/adminHead.php"); ?>
 <?php include("includes/adminMenu.php"); ?>
   ...
   <h4 class="page-head-line">Commercial Subjects REGISTRATION FORM</h4>
  <h2> <a href="reg_new_comm_students.php">Click to register a new student</a></h2> </br></br>
 <?php
  if(isset($_POST['submit'])){
    ...
    $sel_comm=mysqli_query($connection,"SELECT * FROM students where dept ='Commercial' ");
    $row=mysqli_fetch_array($sel_comm);
        $reg_no=$row['matricno'];
        $class=$row['class'];
  ...
  if(mysqli_num_rows($sel) >= 1) {
   ...
   while($row=mysqli_fetch_array($sel_comm)){
    $reg_no=$row['matricno'];
    $class=$row['class'];
    $fullname=$row['firstname'].'  '.$row['middlename'].'  '.$row['lastname'];
...
<div class="row">  
 ...
 $query =$connection->query("SELECT * FROM sss_comm_subj"); 
