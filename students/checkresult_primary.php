...
$sn++; 
 ...
 ?>  
  ...
  echo"<tr>" ;  
         echo "<P>";
               ?>
            <td></td><td><?php //$stdresult=mysqli_query($connection,"select * FROM primary_register

//where   dept='".$_POST['dept']."' AND term='".$_POST['term']."'  AND year='".$_POST['session']."' GROUP BY  regno"); 
// echo $count = mysqli_num_rows($stdresult);?></td> 
 
  <td>NO. IN CLASS:</td><td><?php $stresult=mysqli_query($connection,"select * FROM primary_register
...
$mksOble=100*$count;
 ...
 ?>    
          ...
          if(!$failure){              
                     echo '<tr>';
                     echo "<div class='comments'>"."<strong>"."Total Marks Obtained:.... "." </strong>".$initalsum . " <strong> "."....Total Marks Obtainable:...."." </strong>".$mksOble=100*$count . "<strong>"."....PERCENTAGE:.... "."</strong>"." ". $percent=round($initalsum/$count,2) ."%"." <h4>".
                     "Class Teacher's Comments:"."</h4>";?> <?php if($percent!=0){ echo classTeacherComment($percent);} echo "<h4>"."Principal's Comments: "."</h4>";?> <?php if($percent!=0){ echo principalComment($percent);};  
